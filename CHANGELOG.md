...
## [unreleased][unreleased]
 - Added LF reader-coupling measurement (`LF_GET_FIELD_INFO` 5016 / `LF_SET_STRONG_MV` 5017). The 125kHz field-appearance count alone proved too permissive to gate a lock on: the LPCOMP that raises it trips at roughly VDD/16, so a reader far too weakly coupled to demodulate the emulated card increments it exactly as a well-coupled one does. The firmware now samples the LF envelope with the SAADC at the point in each emulation burst where the modulator is already released, and reports peak and latest level in millivolts alongside sample counts, the longest unbroken *strong* stretch in milliseconds, the longest unbroken field session and the frames sent. Duration is no substitute for strength here — one burst is ten EM4100 frames (~328ms), so even a marginal one-second exposure sends far more frames than a decode needs. A conversion the ADC could not serve is reported as a skipped sample rather than scored as a weak one, and the reply flags whether the envelope channel was claimed at all and whether any sample hit ADC full scale, so an unmeasurable device is distinguishable from a weakly-coupled reader.
